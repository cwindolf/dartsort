...
def merge_clusters(
    spike_index, labels, x, z, ptps, geom_array, denoiser, device
):
...
def get_templates(
    ...
    for unit in tqdm(range(n_templates)):
        ...
        wfs_unit = read_waveforms(
            spike_times_unit[idx], standardized_path, geom_array, n_times
        )[0]
...
def get_diptest_value(
    ...
    n_wfs_max = int(
        min(500, min(n_spikes_templates[unit_a], n_spikes_templates[unit_b]))
    )
    if unit_b == unit_shifted:
        spike_index_unit_a = spike_index[
            labels == unit_a, 0
        ]  # denoiser offset ## SHIFT BASED ON TEMPLATES ARGMIN PN MAX PTP TEMPLATE
        spike_index_unit_b = (
            spike_index[labels == unit_b, 0] + two_units_shift
        )  # denoiser offset
    else:
        spike_index_unit_a = (
            spike_index[labels == unit_a, 0] + two_units_shift
        )  # denoiser offset ## SHIFT BASED ON TEMPLATES ARGMIN PN MAX PTP TEMPLATE
        spike_index_unit_b = spike_index[
            labels == unit_b, 0
        ]  # denoiser offset
    idx = np.random.choice(
        np.arange(spike_index_unit_a.shape[0]), n_wfs_max, replace=False
    )
    spike_times_unit_a = spike_index_unit_a[idx]
    idx = np.random.choice(
        np.arange(spike_index_unit_b.shape[0]), n_wfs_max, replace=False
    )
    spike_times_unit_b = spike_index_unit_b[idx]
    mc = min(384 - n_channels_half, mc)
    mc = max(n_channels_half, mc)
    wfs_a = read_waveforms(
        spike_times_unit_a,
        standardized_path,
        geom_array,
        n_times,
    ...
    wfs_diptest = np.concatenate((wfs_a, wfs_b)).reshape(
...
def get_merged(
    ...
    templates = get_templates(
        standardized_path, geom_array, n_templates, spike_index, labels
    )
