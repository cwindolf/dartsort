...
class RESIDUAL(object):
    ...
    def __init__(self, 
        ...
        print(
            "Instantiating RESIDUAL on ",
            T_sec,
            "seconds long recording"
        )
    ...
    def subtract_parallel(self, batch_ids, fnames_out):
        ...
        for batch_id, fname_out in zip(batch_ids, fnames_out):
            ...
            idx_in_chunk = np.where(
                np.logical_and(spike_train[:,0]>=load_start,
                               spike_train[:,0]<load_end-self.buffer))[0]
            spikes_in_chunk = spike_train[idx_in_chunk]
            # offset
            spikes_in_chunk[:,0] = spikes_in_chunk[:,0] - load_start + pad_left
...
def run_residual(templates_up_path, 
                 spike_train_up_path, 
                 output_directory,
                 standardized_bin,
                 geom_path,
    ...
    # get residual object
    residual_object = RESIDUAL(templates_up_path,
                               spike_train_up_path,
                               standardized_bin,
                               output_directory,
                               geom_path,
