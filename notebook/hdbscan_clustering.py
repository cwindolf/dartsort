...
data_name = 'NP2_Nick_data_new_deduplication'
data_dir = data_path + data_name + '/'
loc_features_path = data_dir 
geom = 'np2_channel_map.npy'
triage_quantile = 75
do_infer_ptp = False
...
data_name = 'NP2_Nick_data_new_deduplication'
data_dir = data_path + data_name + '/'
bin_file = '/media/cat/data/standardized_no_channel_removal.bin'

# %% [markdown]
# ## Load data

# %%
data_path = loc_features_path
...
geom_name = geom
geom_array = np.load(data_dir+geom_name)
...
# %% [markdown]
# ## Triage using Hyun's method

# %%
#perform triaging 
triaged_x, triaged_y, triaged_z, triaged_alpha, triaged_maxptps, triaged_pcs, ptp_filter, idx_keep = run_weighted_triage(x, y, z, alpha, maxptps, pcs, threshold=triage_quantile)
...
# %% [markdown]
# ## Create feature set for clustering (num_spikes_cluster, x and z locations)
...
# %% [markdown]
# ## Perform hdbscan clustering and save results
...
# %% [markdown]
# ## Plot full array and agreement matrix
...
fig, axes = plt.subplots(1, 3, sharey=True, figsize=(16, 24))
