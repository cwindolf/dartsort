...
# %%
#parameters
bath_path = '/moto/stats/users/ch3676/recordings2/'
recording_name = 'Neuropixels-64_rigid_uniform_homogeneous'
recording_dir=f'{recording_name}_preprocessed/'
data_path = bath_path + recording_dir
sorting_name = recording_name + '.h5'
...
sort_gt = si.MEArecSortingExtractor(sort_path)
...
# %%
name_extension = "_julien_merge"

output_folder = Path("/local/bincache/") / f"Neuropixels-64_simulated_results/{recording_name}{name_extension}"
...
args = {}
#hyperparams
args["do_reg"] = do_reg = True
args["do_superres"] = do_superres = True
...
args["merge_thresh_early"] = merge_thresh_early = 2.0 # 2.0
args["merge_thresh_end"] = merge_thresh_end = 2.0
args["deconv_thresh"] = deconv_thresh = 200 
...
args["do_reloc"] = do_reloc = do_reg #this should be true if you are registering
args["fs"] = fs = 32000
args["localize_radius"] = localize_radius = 200

#clustering
args["min_cluster_size"] = min_cluster_size = 15
args["cluster_selection_epsilon"] = cluster_selection_epsilon = 20.0

#ensembling
args["do_ensemble"] = do_ensemble = False
if do_ensemble:
    args["num_ensemble"] = num_ensemble = 2
    args["ensemble_percent"] = ensemble_percent = .8

#drift-aware merge
args["drift_merge"] = drift_merge = True
args["zero_radius_um"] = zero_radius_um = 70
args["bin_size_um"] = bin_size_um = pitch // 4
args["drift_merge_thresh"] = drift_merge_thresh = 1.0
...
# with h5py.File("/media/peter/2TB/sherry/IND106_shank1_Mar26/sub/subtraction.h5", "r+" if overwrite else "r") as h5:
    ...
    if "z_reg" in h5 and not overwrite:
        ...
        if do_reg:
            ...
            print("registering...")
            tme, extra = newton_motion_est.register(
                maxptp,
                z,
                t,
                raster_kw=dict(gaussian_smoothing_sigma_um=3), #3
                weights_kw=dict(
                    weights_threshold_low=0.2,
                    weights_threshold_high=0.2,
                    mincorr=0.1,
                ),
                thomas_kw=dict(eps=1e-5),
                max_disp_um=100,
                rigid=True,
            )
            z_reg = tme.correct_s(t, z)
            p = tme.displacement
        else:
            z_reg = z_abs
    ...
    #plot
    raster, dbe, tbe = mu.fast_raster(maxptp, z, t)
    mu.show_raster(raster, dbe, tbe, plt)
    plt.show()
    mu.show_registered_raster(tme, maxptp, z, t, plt, vmax=15, aspect="auto")
...
# %%
st = spike_index.copy()
st[:, 1] = np.linalg.norm(geom[None, :, :] - np.c_[x, z_reg][:, None, :], axis=2).argmin(1)
good_times = np.isin((st[:, 0]) // fs, np.flatnonzero(p_good))
print(f"{good_times.sum()=}")
st[~good_times, 1] = -1
print("NOT ENSEMBLING")
spike_train, templates, order = newms.new_merge_split(
    ...
    relocated=do_reloc,
    drift_merge=drift_merge,
    zero_radius_um=zero_radius_um,
    bin_size_um=bin_size_um,
    sampling_rate=fs,
    trough_offset=trough_offset,
    spike_length_samples=spike_length_samples,
    threshold_resid=drift_merge_thresh,
    split_kwargs=dict(
        split_steps=(
...
plt.xlabel("time (s)")
plt.ylabel("depth (um)")
plt.title(f"split/merge time vs. registered y.  {kept.sum()} sorted spikes.")
...
plt.xlabel("time (s)")
plt.ylabel("depth (um)")
plt.title(f"split/merge time vs. y.  {kept.sum()} sorted spikes.")
...
plt.ylabel("depth (um)")
plt.title(f"split/merge time vs. y.  {kept.sum()} sorted spikes.")
fig.savefig(dsout / "scatterplots" / "deconv1_scatter_sorted_t_v_y.png", dpi=300)
...
for k in ("split", "merge"):
    ...
    od = dsout / "summaries"
    vissort.make_unit_summaries(out_folder=od / f"{vissort.name_lo}_raw", units=[20,21])
...
if do_superres:
    deconv_dict = drifty_deconv.superres_deconv(
        raw_data_bin,
        geom,
        z,
        p,
...
if do_superres:
    extract_deconv1_h5, extract_deconv1_extra = drifty_deconv.extract_superres_shifted_deconv(
        ...
        nn_denoise=do_neural_net_denoise,
        geom=geom,
        n_jobs=20,
        # save_reassignment_residuals=True,
        # pairs_method="radius",
        max_resid_dist=20, #what is going on here?
...
# %%
# merge_order = np.load(dsout / "de" / "merge_order.npy")
...
plt.xlabel("time (s)")
plt.ylabel("depth (um)")
plt.title(f"deconv1, time vs. registered y.  {kept.sum()} sorted spikes.")
fig.savefig(dsout / "scatterplots" / "deconv1_scatter_sorted_t_v_regy.png", dpi=300)
...
plt.xlabel("time (s)")
plt.ylabel("depth (um)")
plt.title(f"deconv1, time vs. y.  {kept.sum()} sorted spikes.")
fig.savefig(dsout / "scatterplots" / "deconv1_scatter_sorted_t_v_y.png")
...
plt.xlabel("time (s)")
plt.ylabel("depth (um)")
plt.title(f"post deconv merge time vs. registered y.  {kept.sum()} sorted spikes.")
...
plt.xlabel("time (s)")
plt.ylabel("depth (um)")
plt.title(f"post deconv merge time vs. y.  {kept.sum()} sorted spikes.")
...
with h5py.File(extract_deconv1_h5) as h5:
    ...
    p = h5["p"][:]
if do_superres:
    deconv_dict_2 = drifty_deconv.superres_deconv(
        raw_data_bin,
        geom,
        z,
        p,
...
# %%
spike_train, order, templates, template_shifts = spike_train_utils.clean_align_and_get_templates(
    np.load(dsout / f"deconv2clust" / "merge_st.npy"),
    geom.shape[0],
    raw_data_bin,
    trough_offset=trough_offset,
    spike_length_samples=spike_length_samples,
...
if do_superres:
    with h5py.File(extract_deconv2_h5) as h5: 
        z = h5["localizations"][:, 2][merge_order]
        p = h5["p"][:]
    deconv_dict_3 = drifty_deconv.superres_deconv(
        raw_data_bin,
        geom,
        z,
        p,
...
np.save(dsout / f"deconv3" / "deconv_spike_train.npy",  deconv_dict_3['deconv_spike_train'])
...
sorting_deconv_final = si.numpyextractors.NumpySorting.from_times_labels(
        times_list=deconv3_spike_train[:,0].astype("int"),
        labels_list=deconv3_spike_train[:,1].astype("int"),
...
# %%
plt.imshow(cmp_gt.get_ordered_agreement_scores().to_numpy())

# %%
# cmp.get_performance()['precision']

# %%
# cmp.get_performance()['precision'].to_numpy()

# %%
for name, cmp in [('final_deconv (rp: 10)', cmp_gt)]:
    well_detected_units = cmp.get_well_detected_units(well_detected_score = .8)
    fig, axes = plt.subplots(1,3, figsize=(18,6))
    axes[0].scatter(snrs.values(), cmp.get_performance()['precision'].to_numpy())
    ...
    axes[1].scatter(snrs.values(), cmp.get_performance()['recall'].to_numpy())
    ...
    axes[2].scatter(snrs.values(), cmp.get_performance()['accuracy'].to_numpy())
...
# %%
# deconv3_spike_train = np.load(Path('/media/cat/cole/simulated_results/outputs_ensemble_static_5_min_sim_1') / f'deconv3' / 'deconv_spike_train.npy')
