...
# %%
recbin = si.read_binary(
    dsout / "sippx" / "traces_cached_seg0.raw",
    fs,
    rec_fly.get_num_channels(),
    np.float32,
    t_starts=[rec_fly.get_times()[0]],
    time_axis=0,
    gain_to_uV=1,
    offset_to_uV=0,
    # is_filtered=True,
...
# %%
folder_traces = rec.get_traces(start_frame=1_000_000, end_frame=1_020_000)
binary_traces = recbin.get_traces(start_frame=1_000_000, end_frame=1_020_000)
fly_traces = rec_fly.get_traces(start_frame=1_000_000, end_frame=1_020_000)

# %%
folder_traces.min(), folder_traces.max()

# %%
binary_traces.min(), binary_traces.max()

# %%
fly_traces.min(), fly_traces.max()

# %%
np.array_equal(folder_traces, fly_traces), np.array_equal(binary_traces, fly_traces)

# %%
rec.has_scaled(), recbin.has_scaled(), rec_fly.has_scaled()

# %%
rec.has_scaled_traces(), recbin.has_scaled_traces(), rec_fly.has_scaled_traces()

# %%
rec.get_sampling_frequency(), recbin.get_sampling_frequency(), rec_fly.get_sampling_frequency()

# %%
rec.get_num_samples(), recbin.get_num_samples(), rec_fly.get_num_samples()
