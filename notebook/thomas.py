...
for pid in pids:
    ff = feats[pid]
    me, extra = newt.register(ff['a'], ff['z'], ff['t'])
...
for pid in pids:
    ...
    me, extra = newt.register(
        ...
        ff['t'],
        max_dt_s=1000,
        thomas_kw=dict(weights_threshold_low=0.25),
    )
    thomas_dt[pid] = dict(me=me, extra=extra)

# %%
thomas_mc1 = {}
for pid in pids:
    ff = feats[pid]
    uextra = thomas_unthresh[pid]["extra"]
    
    me, extra = newt.full_weighted_thomas(
        ff['a'],
        ff['z'],
        ff['t'],
        uextra["D"],
        ...
        bin_um=1,
        bin_s=1,
        windows=uextra["windows"],
        window_centers=uextra["window_centers"],
        raster_kw=dict(
            amp_scale_fn=None,
    ...
    thomas_mc1[pid] = dict(me=me, extra=extra)
...
for pid in pids:
    ...
    me, extra = newt.full_weighted_thomas(
        ...
        mincorr=0.1,
        max_dt_s=1000,
        weights_threshold_low=0.25
    )
    
    thomas_dt_thresh[pid] = dict(me=me, extra=extra)
...
for pid in pids:
    ...
    thomas_mc2[pid] = dict(me=me, extra=extra)
...
for pid in pids:
    ...
    me, extra = newt.full_weighted_thomas(
        ...
        pbar=True,
        mincorr=0.2,
        weights_threshold_low=(500, 5),
        weights_threshold_high=(2000, 5),
    )
    
    thomas_mc1_thresh[pid] = dict(me=me, extra=extra)
...
for pid in pids:
    ...
    me, extra = newt.full_weighted_thomas(
        ...
        pbar=True,
        mincorr=0.2,
        weights_threshold_low=(250, 5),
        weights_threshold_high=(1000, 5),
    )
    
    thomas_mc1_thresh2[pid] = dict(me=me, extra=extra)
...
for pid in pids:
    ...
    me, extra = newt.full_weighted_thomas(
        ...
        pbar=True,
        mincorr=0.2,
        weights_threshold_low=(1000, 5),
        weights_threshold_high=(1000, 5),
    )
    
    thomas_mc1_thresh3[pid] = dict(me=me, extra=extra)
...
for pid in pids:
    ...
    thomas_mc1_thresh4[pid] = dict(me=me, extra=extra)
...
for pid in pids:
    ...
    thomas_mc1_thresh5[pid] = dict(me=me, extra=extra)
...
for pid in pids:
    ...
    for pos in me.spatial_bin_centers_um:
        ls, = aa.plot(tt[:-1], pos + offset + me.disp_at_s(tt[:-1], depth_um=pos), color="k")
...
for pid in pids:
    ...
    for name, meme, color in toplot:
        me = meme[pid]["me"]
        for pos in me.spatial_bin_centers_um:
            ls, = aa.plot(tt[:-1], pos + offset + me.disp_at_s(tt[:-1], depth_um=pos), color=color)
        offset += 40
        names.append(name)
        handles.append(ls)
...
def get_weights(windows, amps, depths, times, me=None, depths_reg=None):
    r, db, tb = ibme.fast_raster(amps, depths, times, avg_in_bin=False, amp_scale_fn=np.log1p)
    if depths_reg is None:
        depths_reg = me.correct_s(times, depths)
    rr, dbr, tbr = ibme.fast_raster(amps, depths_reg, times, avg_in_bin=False, amp_scale_fn=np.log1p)
...
for pid, cdsr in cdsrs.items():
    ...
    me = motion_est.NonrigidMotionEstimate(ps, spatial_bin_centers_um=win_centers, time_bin_edges_s=tbe)
...
for pid in pids:
    ...
    me = motion_est.NonrigidMotionEstimate(rs, time_bin_edges_s=newt_me.time_bin_edges_s, spatial_bin_centers_um=newt_me.spatial_bin_centers_um)
...
for pid, cdsr in cdsrs.items():
    ...
    win_centers = origregs[pid]["extra"]["window_centers"]
    tbe = origregs[pid]["extra"]["time_bin_edges_s"]
    me = motion_est.NonrigidMotionEstimate(P, spatial_bin_centers_um=win_centers, time_bin_edges_s=tbe)
