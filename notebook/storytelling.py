...
# %%
fig, (aa, ab) = plt.subplots(2, 1, figsize=(5, 10))
cuts.plot(R0, ax=aa)
...
feats_orig = np.ascontiguousarray(np.c_[xs, ys, z_reg, alphas, loadings_orig[:, :2]][good]).T
feats_yza = np.ascontiguousarray(np.c_[xs, ys, z_reg, alphas, loadings_yza[:, :2]][good]).T
feats_xyza = np.ascontiguousarray(np.c_[xs, ys, z_reg, alphas, loadings_xyza[:, :2]][good]).T
...
with timer("isosplit"):
    ...
    labels_orig = isosplit(feats_orig[:, shuffle], K_init=512)
...
with timer("isosplit"):
    ...
    labels_yza = isosplit(feats_yza[:, shuffle], K_init=512)
...
with timer("isosplit"):
    ...
    labels_xyza = isosplit(feats_xyza[:, shuffle], K_init=512)
