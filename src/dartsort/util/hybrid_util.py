...
def sorting_from_times_labels(times, labels, recording=None, sampling_frequency=None,  determine_channels=True, template_config=unshifted_raw_template_config, with_locs=False, n_jobs=0):
    ...
    labels_flat = labels.copy()
    _, labels_flat[labels>-1] = np.unique(labels[labels>-1], return_inverse=True)
    ...
    td = TemplateData.from_config(recording, sorting, template_config, with_locs=with_locs, n_jobs=n_jobs)
...
def sorting_from_spikeinterface(sorting, recording=None, determine_channels=True, template_config=unshifted_raw_template_config, with_locs=False, n_jobs=0):
    sv = sorting.to_spike_vector()
    return sorting_from_times_labels(sv['sample_index'], sv['unit_index'], sampling_frequency=sorting.sampling_frequency, recording=recording, determine_channels=determine_channels, template_config=template_config, with_locs=with_locs, n_jobs=n_jobs)
