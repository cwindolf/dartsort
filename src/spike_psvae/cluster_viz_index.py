...
def ploc(
    ...
    """Plot single channel waveforms according to localization on probe"""
    
    ax = ax or plt.gca()

    T = waveforms.shape[1]
    # if waveforms.shape != (*xz.shape[0], T):
    #     raise ValueError(
    #         f"Bad shapes: {waveforms.shape=}, {x.shape[0]=}"
    #     )
    ...
    # xz_plot = geom * geom_scales
    xz_plot = xz * geom_scales
    t_domain = np.linspace(-T // 2, T // 2, num=T)
    ...
    for c, wf in enumerate(waveforms):
        draw.append(xz_plot[c, 0] + t_domain)
        draw.append(xz_plot[c, 1] + wf)
        xmin = min(xz_plot[c, 0] + t_domain.min(), xmin)
        xmax = max(xz_plot[c, 0] + t_domain.max(), xmax)
        unique_chans.add(c)
    dx = xmax - xmin
    ax.set_xlim(
        [
    ...
    #         if show_zero_kwargs is None:
    #             show_zero_kwargs = dict(color="gray", lw=1, linestyle="--")
    #         ax.axhline(xz_plot[c, 1], **show_zero_kwargs)
    #     if show_chan_label:
    #         ax.annotate(
    #             chan_labels[c], xz_plot[c] + ann_offset, size=6, color="gray"
    #         )
    linestyle = linestyle or ls
    lines = ax.plot(
        *draw,
    ...
    if subar:
        min_z = min(xz_plot[:, 1])# for c in unique_chans)
        if msbar:
            min_z += max_abs_amp
        ax.add_patch(
            Rectangle(
    ...
    if zlim is None:
        ...
        min_z = min(xz_plot[:, 1])# for c in unique_chans)
        max_z = max(xz_plot[:, 1])# for c in unique_chans)
        if np.isfinite([min_z, max_z]).all():
            ax.set_ylim([min_z - 2 * max_abs_amp, max_z + 2 * max_abs_amp])
    elif zlim == "tight":
        min_z = min(xz_plot[:, 1])# for c in unique_chans)
