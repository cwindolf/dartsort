...
#     ]
    ...
    with ctx.Pool(
        ...
        for res in tqdm(
            pool.imap_unordered(
                mp_object.filter_standardize_batch_mp_restandardize,
                all_batches,
            ),
            total=len(all_batches),
            desc="Preprocessing",
...
class multi_proc_object:
    ...
    def filter_standardize_batch_mp_restandardize(self, batch_id):
        filter_standardize_batch_restandardize(
            batch_id, self.filename_raw, 
            self.fname_mean_sd,
            self.apply_filter,
            self.dtype_raw, 
...
def filter_standardize_batch_restandardize(batch_id, bin_file, fname_mean_sd,
    ...
    fname_mean_sd_old = fname_mean_sd[:-31] + "old_" + fname_mean_sd[-31:]
    old_mean_sd = np.load(fname_mean_sd_old)
    old_centers = old_mean_sd["centers"]
    old_sd = np.median(old_mean_sd["sd"])
...
def restandardize_get_std(ts,
    ...
    fname_old = fname[:-31] + "old_" + fname[-31:]
    print(fname_old)
    mean_sd = np.load(fname)
    centers = mean_sd["centers"]
    sd = np.median(mean_sd["sd"])

    np.savez(fname_old,
         centers=centers,
         sd=sd)

    ts = ts * sd + centers[None, :]
