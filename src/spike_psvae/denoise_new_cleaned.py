...
def multichan_phase_shift_denoise_preshift(waveforms, ci_graph_all_maxCH_uniq, maxCH_neighbor, Denoiser, maxchans, device, offset=42):
    ...
    waveforms_roll_all = waveforms_roll_all.permute(0, 2, 1).reshape(N, T, S, C).permute(0, 1, 3, 2)                             
    ...
    real_maxCH_info = waveforms_ptp_denoise[row_idx, torch.flatten(col_idx), 0].reshape(N, 8)
    real_maxCH_info = torch.max(real_maxCH_info, dim = 1) #both max and argmax
    real_maxCH_idx = real_maxCH_info[1]
    real_maxCH = col_idx[range(N), real_maxCH_idx].long()
    
    #positive or negative spike omn the max channel
    ...
    while True:
        if all(len(v)==0 for v in Q):
            return torch.einsum("itjk,ijk->itj", waveforms_roll_denoise, pick_idx)[:, :, range(ci_graph_all_maxCH_uniq.shape[1])]#, waveforms_roll_denoise[:,:,range(40),0]               
        u = Q.pop()
        ...
        #√√√√√√√
        #Torch.einsum PROBABLY NOT the fastest for selecting a specific column
