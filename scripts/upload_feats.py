...
rwfs, firstchans_std, _ = waveform_utils.relativize_waveforms_np1(
    sub["cleaned_waveforms"][:], sub["first_channels"][:], sub["geom"][:], sub["spike_index"][:, 1], feat_chans=20
)

np.save(out_folder / "ae_features.npy", feats["features"])
np.save(
    out_folder / "localization_results.npy",
    np.c_[ls[:, 0], ls[:, 3], ls[:, 1], ls[:, 4], locs["maxptp"], firstchans_std, sub["spike_index"][:, 1]],
)
