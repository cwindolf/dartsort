...
if __name__ == "__main__":
    ...
    if Path(args.pids).exists():
        pids = open(args.pids).read().split()
    else:
    ...
    for pid in args.pids:
        ...
        do_anything = (not (sessdir / "ks_relocalizations.npz").exists()) or (not (sessdir / "subtraction.h5").exists())
        if not do_anything:
            continue
        ...
        if args.ksreloc and not (sessdir / "ks_relocalizations.npz").exists():
            ...
            aligned_spike_train, order, templates_aligned, template_shifts = clean_align_and_get_templates(
                spike_train, 
            ...
            for j in np.unique(aligned_spike_train[:,1]):
                aligned_spike_index[aligned_spike_train[:,1]==j, 1] = templates_aligned[j].ptp(0).argmax()
