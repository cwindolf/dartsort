...
def main():
    ...
    kilo_spike_frames = (kilo_spike_samples - 30*60*30000) + 18 #+18 to match our detection alignment
    ...
    if args.do_save_figures:
        ...
        #save cluster indices
        if not os.path.exists(save_dir_path + '/match_geq_50'):
            os.makedirs(save_dir_path + '/match_geq_50')
        if not os.path.exists(save_dir_path + '/match_geq_10_l_50'):
        ...
        for cluster_id in pbar:
            if cluster_id != -1:
                ...
                if cmp is not None:
                    num_channels = triaged_wfs_localized.shape[2]
                    fig = plot_agreement_venn(cluster_id, cmp, sorting_hdbl_t, sorting_kilo, geom_array, num_channels, args.num_spikes_plot, triaged_firstchans[clusterer.labels_ == cluster_id], 
