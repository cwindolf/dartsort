...
geom_path = '/media/cat/julien/spikes_localization_registration/channels_maps/np1_channel_map.npy'
...
def split_individual_cluster(standardized_path, spike_index_unit, x_unit, z_unit, ptps_unit, denoiser, device, n_channels = 10):
...
def split_clusters(standardized_path, spike_index, labels, x, z, ptps, denoiser, device):
    labels_new = labels.copy()
    labels_original = labels.copy()
    ...
    for unit in (np.unique(labels)[1:]):
        spike_index_unit = spike_index[labels == unit]
        x_unit, z_unit, ptps_unit = x[labels == unit], z[labels == unit], ptps[labels == unit]
        is_split, unit_new_labels = split_individual_cluster(standardized_path, spike_index_unit, x_unit, z_unit, ptps_unit, denoiser, device)
        if is_split:
            for new_label in np.unique(unit_new_labels):
                if new_label == -1:
                    idx = np.flatnonzero(labels_original == unit)[unit_new_labels == new_label]
...
def get_diptest_value(standardized_path, spike_index, labels, unit_a, unit_b, n_spikes_templates, mc, two_units_shift, unit_shifted, n_channels = 10):
    ...
    wfs_b = read_waveforms(spike_times_unit_b, standardized_path, geom_array, n_times=121, channels = np.arange(mc-n_channels_half,mc+n_channels_half))[0]
...
def get_merged(standardized_path, n_templates, spike_index, labels, x, z, n_temp = 5, distance_threshold = 2., threshold_diptest = 1.25):
...
labels_split = split_clusters(standardized_path, spike_index_triaged, ordered_labels, triaged_x, triaged_z, triaged_maxptps, denoiser, device)
